 ...
 */
public class DoSomethingAbsurdToABunchOfPoints extends EvalFunc<DataBag> {
    ...
    private List<GeoFeature> getKCenters(String qualifier, String quadkey, List<GeoFeature> points, Integer k) throws ExecException {
        List<GeoFeature> existingClusters = new ArrayList<GeoFeature>(k.intValue());
        ...
        for (GeoFeature point : points) {
            ...
            if ((point.getProperties().has(TYPE_KEY) && point.getProperties().getString(TYPE_KEY).equals(CLUSTER_TYPE)) || point.getProperties().has(CLUSTER_KEY)) isCluster = true;
            if (isCluster) {
                existingClusters.add(point);
                kCenters.remove(point);
            } 
        }

        existingClusters = existingClusters.subList(0, Math.min(k.intValue(), existingClusters.size()));
        if (existingClusters.size() == k.intValue()) return existingClusters;
        
        Collections.shuffle(kCenters); // so we can take K centers off the top

        int i = 0;
        while (existingClusters.size() < k.intValue()) {
